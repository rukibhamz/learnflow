...
@section('content')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @foreach($guides as $guide)
                <div class="bg-surface border border-rule rounded-xl p-6 hover:border-accent/30 transition-all group">
                    <div class="w-12 h-12 bg-{{ $guide['color'] }}/10 flex items-center justify-center rounded-xl mb-5">
                        <span class="material-symbols-outlined text-{{ $guide['color'] }} text-[24px]">{{ $guide['icon'] }}</span>
                    </div>
                    <div class="flex items-center gap-2 mb-3">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-ink3 bg-bg px-2 py-0.5 rounded">{{ $guide['tag'] }}</span>
                    </div>
                    <h3 class="font-display font-bold text-base text-ink mb-2 group-hover:text-accent transition-colors">{{ $guide['title'] }}</h3>
                    <p class="text-sm text-ink2 leading-relaxed">{{ $guide['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <section id="tutorials" class="bg-surface border-y border-rule">
        <div class="max-w-7xl mx-auto px-6 py-20">
            <div class="flex items-end justify-between mb-10">
                ...
                <a href="{{ route('courses.index') }}" class="text-sm font-bold text-accent hover:opacity-80 transition-opacity hidden md:block">View All Courses &rarr;</a>
            </div>
                    ...
                    <div class="flex gap-5 p-5 bg-bg rounded-xl border border-rule/50 hover:border-accent/30 transition-all group">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-[10px] font-bold text-ink3 uppercase tracking-widest">{{ $tut['duration'] }}</span>
                            </div>
                            <h3 class="font-display font-bold text-[15px] text-ink mb-1 group-hover:text-accent transition-colors">{{ $tut['title'] }}</h3>
                            <p class="text-sm text-ink2 leading-relaxed line-clamp-2">{{ $tut['desc'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @foreach($tools as $tool)
                <div class="bg-surface border border-rule rounded-xl p-6 text-center hover:border-accent/30 transition-all group">
                    <div class="w-14 h-14 bg-accent/5 flex items-center justify-center rounded-xl mx-auto mb-4 group-hover:bg-accent/10 transition-colors">
                        <span class="material-symbols-outlined text-accent text-[28px]">{{ $tool['icon'] }}</span>
                    </div>
                    <h4 class="font-display font-bold text-sm text-ink mb-1">{{ $tool['title'] }}</h4>
                    <p class="text-xs text-ink3 leading-relaxed">{{ $tool['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-6 py-20">
        <div class="bg-ink p-12 lg:p-20 flex flex-col lg:flex-row items-center gap-12">
            ...
            <a href="{{ route('pages.contact') }}" class="bg-accent text-white font-bold px-10 py-4 rounded-card whitespace-nowrap hover:opacity-90 transition-opacity text-base">
                Contact Support
            </a>
        </div>
    </section>
